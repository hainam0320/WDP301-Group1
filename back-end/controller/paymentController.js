    ...
    try {
        ...
        if (secureHash === signed) {
            ...
            if (order.paymentStatus === 'paid' && rspCode === '00') {
                return res.status(200).json({ RspCode: '02', Message: 'Order already confirmed success' });
            }
            ...
            if (rspCode === '00') {
                ...
                // Giao dịch thất bại
                companyTransaction.status = 'disputed'; // Giao dịch có vấn đề
                companyTransaction.remarks = `Thanh toán VNPAY thất bại. Mã lỗi: ${rspCode}`;
                companyTransaction.processed_at = new Date();
                companyTransaction.transactionRefId = vnpTransactionNo;
                companyTransaction.type = 'user_payment_held'; // Vẫn giữ loại này, nhưng trạng thái disputed
                await companyTransaction.save();

                order.paymentStatus = 'disputed_payment';
                order.status = 'payment_failed'; // Thanh toán thất bại
                order.vnpayTransactionId = vnpTransactionNo;
                await order.save();

                return res.status(200).json({ RspCode: '00', Message: 'Failed' });
            }
        } else {
            console.error('VNPAY Callback: Invalid SecureHash');
            // Cập nhật transaction và order là có tranh chấp nếu hash không khớp
            companyTransaction.status = 'disputed';
            companyTransaction.remarks = 'SecureHash không hợp lệ từ VNPAY Callback';
            companyTransaction.processed_at = new Date();
            companyTransaction.transactionRefId = vnpTransactionNo;
            companyTransaction.type = 'user_payment_held';
            await companyTransaction.save();
            ...
            return res.status(200).json({ RspCode: '97', Message: 'Invalid SecureHash' });
        }
    } catch (error) {
